/*
Módulo: ativacaoTanh (Função Tangente Hiperbólica)
Descrição:

    Este módulo implementa uma aproximação da função de ativação tangente
    hiperbólica (tanh) utilizando aritmética de ponto fixo em 16 bits.

    A função tanh é definida como:
        tanh(x) = (e^x - e^-x) / (e^x + e^-x)

    Para viabilizar sua implementação eficiente em hardware, é utilizada uma
    aproximação por segmentos lineares (piecewise linear approximation),
    evitando o uso de operações exponenciais custosas.

    O domínio da função é particionado em intervalos, sendo que em cada
    intervalo a função é aproximada por uma reta definida a partir de pontos
    previamente calculados. As inclinações são implementadas com deslocamentos
    (shift) e combinações simples, reduzindo o custo computacional.

    Para valores negativos, utiliza-se a propriedade de simetria ímpar:
        tanh(-x) = -tanh(x)

    A saída é saturada no intervalo [-1, 1].

Formato numérico:
    - Representação em ponto fixo Q4.12 (16 bits)
    -  1.0 = 0x1000
    - -1.0 = 0xF000

Entradas:
    d_in  : Valor de entrada (Q4.12, com sinal)

Saídas:
    d_out : Valor de saída aproximado da tanh (Q4.12)
*/
module ativacaoTanh (
