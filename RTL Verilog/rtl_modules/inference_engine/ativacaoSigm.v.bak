/*
Módulo: ativacaoSigm (Função Sigmoide)
Descrição:

    Este módulo implementa uma aproximação da função de ativação sigmoide
    utilizando aritmética de ponto fixo em 16 bits.
    ...
    previamente calculados.Sendo dividido em intervalos,
	 e em cada intervalo a saída é estimada por:
        σ(x) ≈ σ(x_i) + slope * (x - x_i)

    As inclinações são implementadas com deslocamentos (shift) e combinações
	 simples, reduzindo o custo computacional.

    Para valores negativos, é utilizada a propriedade:
        σ(-x) = 1 - σ(x)

    A saída é saturada no intervalo [0, 1].
...
Saídas:
    d_out : Valor de saída aproximado da sigmoide (Q4.12)
*/
module ativacaoSigm (
