 ...
 */
int convert_png_to_raw(const char* png_path, const char* raw_path) {
    ...
    if (!f) {
        fprintf(stderr, "Erro: Falha ao criar o arquivo RAW de saida '%s'\n", raw_path);
        stbi_image_free(imagem);
        return -1;
    }

    // Grava o buffer contínuo (28 * 28 = 784 bytes)
    fwrite(imagem, 1, largura * altura, f);
