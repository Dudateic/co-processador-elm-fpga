...
def mif_to_bin_little_endian(mif_path, bin_path, word_bytes=2):
    ...
    with open(mif_path, "r") as f:
        for linha in f:
            ...
            # 15 : FF;
            # etc
            encontrados = re.findall(r'([0-9A-Fa-f]+)\s*;', linha)

            if encontrados:
                valor_hex = encontrados[-1]
...
if __name__ == "__main__":
    ...
    match len(sys.argv):
        case 2:
            mif_to_bin_little_endian(sys.argv[0], sys.argv[1])
        case 3:
            try:
                bytes = int(sys.argv[2])
                mif_to_bin_little_endian(sys.argv[0], sys.argv[1], bytes)
            except:
                print("Erro: Argumento 3 precisa ser um valor.")
        case _:
